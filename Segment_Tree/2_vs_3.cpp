...
void build_tree(int *tree, int *arr, int start, int end, int treeIndex){
    if(start == end){
        tree[treeIndex] = arr[start];
        return;
    }
        int mid = (start+end)/2;
        build_tree(tree, arr, start, mid, 2*treeIndex);
        build_tree(tree, arr, mid+1, end, 2*treeIndex+1);
        tree[treeIndex] = (power[end-mid]*tree[2*treeIndex] + tree[2*treeIndex+1])%3;
}
...
int query(int *tree, int start, int end, int treeIndex, int left, int right){
    ...
    int mid = (start+end)/2;
    int x = query(tree, start, mid, 2*treeIndex, left, right);
    int y = query(tree, mid+1, end, 2*treeIndex+1, left, right);
    return (x+y)%3;
}

void update_tree(int *tree, int *arr, int start, int end, int treeIndex, int key){
    if(start == end){
        arr[key] = 1;
        tree[treeIndex] = 1;
        return;
        ...
        int mid = (start+end)/2;
        if(start <= key && mid >= key) 
            update_tree(tree, arr, start, mid, 2*treeIndex, key);
        else 
            update_tree(tree, arr, mid+1, end, 2*treeIndex+1, key);
...
int main(){
    ...
    while(q--){
        ...
        if(type == 0){
            ...
            cin >> key;
            if(arr[key] == 0) 
                update_tree(tree, arr, 0, n-1, 1, key);
        }
    }
    return 0;   
}
