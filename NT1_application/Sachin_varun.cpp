...
triplet gcd_extended_euclid(ll a, ll b){
    if(b == 0){
    	triplet my_ans;
        my_ans.x = 1;
        my_ans.y = 0;
    ...
    triplet small_ans = gcd_extended_euclid(b, a%b);
    triplet my_ans;
    my_ans.x = small_ans.y;
    my_ans.y = small_ans.x - ((a/b)*(small_ans.y));
    my_ans.gcd = small_ans.gcd;
    return my_ans;
}
ll mod_inverse(ll A, ll M){
    ll x = gcd_extended_euclid(A,M).x;
    return (x%M + M)%M;
}
int main(){
	int t;
